...
test.describe('Blog posts', () => {
  test('should render blog post with banner image', async ({ page }) => {
    ...
    // Verify banner image. The eleventy-img transform rewrites the authored
    // `/assets/images/...` src into a generated, content-hashed <picture>
    // source set, so assert on the responsive wiring rather than a fixed path.
    const banner = page.locator('div.relative img[itemprop="image"]');
    await expect(banner).toBeVisible();
    await expect(banner).toHaveAttribute('src', /^\/assets\/img\/.+\.(png|jpe?g|webp)$/);
    await expect(banner).toHaveAttribute('srcset', /\d+w/);
    await expect(banner).toHaveAttribute('sizes', /\d+px/);
    await expect(banner).toHaveAttribute('alt', 'Generated by Gemini 2.5-flash');
    await expect(banner.locator('xpath=..')).toHaveJSProperty('tagName', 'PICTURE');
    await expect(page.locator('div.relative picture source[type="image/webp"]')).toHaveAttribute(
      'srcset',
      /\.webp \d+w/
    );
