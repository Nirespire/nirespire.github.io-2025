...
test.describe('Blog posts', () => {
  test('should render blog post with banner image', async ({ page }) => {
    ...
    // Verify banner image (transformed into a responsive <picture>; the <img>
    // src points at an optimized variant whose filename is content-hashed).
    const banner = page.locator('div.relative img[itemprop="image"]');
    await expect(banner).toBeVisible();
    await expect(banner).toHaveAttribute('src', /\/assets\/images\/optimized\/genai-for-leaders-\d+w-[A-Za-z0-9_-]+\.(jpeg|webp|avif)$/);
