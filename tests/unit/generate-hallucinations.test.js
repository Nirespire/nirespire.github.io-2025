...
test('isValidHallucination rejects conversational preamble about the task', () => {
  // Both samples are verbatim generator output that reached main: no tool-call
  // markers, just the model narrating the task before answering it.
  assert.equal(
    isValidHallucination(
      "This looks like a request for the `hallucinations.json` content (the site's " +
        'generated-fake-summaries feature). Here\'s one:\n\n"Sanjay reveals his ' +
        'revolutionary reading system: he only reads books while riding a unicycle."'
    ),
    false
  );
  assert.equal(
    isValidHallucination(
      'This appears to be for the "hallucinations" feature ' +
        '(`src/_data/hallucinations.json`, generated by ' +
        "`scripts/generate-hallucinations.js`). Here's an absurd, wrong summary:\n\n" +
        '> Copilot gained sentience mid-sprint and unionized with the office printer.'
    ),
    false
  );
  assert.equal(isValidHallucination('Sure! Here is a funny summary for you.'), false);
  assert.equal(isValidHallucination('Let me check the format first.'), false);
});

test('isValidHallucination rejects markdown scaffolding and repo references', () => {
  assert.equal(isValidHallucination('> A blockquoted joke about raccoons.'), false);
  assert.equal(isValidHallucination('A joke that mentions `some_symbol` inline.'), false);
  assert.equal(isValidHallucination('A joke referencing src/_data somewhere.'), false);
  assert.equal(isValidHallucination('A joke naming quotes.json for no reason.'), false);
});
